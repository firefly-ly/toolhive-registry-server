...
function TrendChart({ days, lines }: { days: string[]; lines: TrendItem[] }) {
  ...
  return (
    ...
    >
      ...
      {hitName && mousePx && (
        <div
          className="pointer-events-none absolute z-20 -translate-x-1/2 -translate-y-full rounded-md border-border bg-popover px-2 py-1 text-xs font-semibold text-popover-foreground shadow-md"
          style={{
          ...
          >
            ...
            {isHover &&
              (() => {
                ...
                const dotW = 7 + 6; // 圆点直径 + 与文字间距
                const w =
                  Math.max(tw(nameTxt, 11), tw(metaTxt, 9.5)) + dotW + 22;
                const h = 38;
