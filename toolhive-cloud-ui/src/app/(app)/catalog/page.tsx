...
export default async function CatalogPage({ searchParams }: CatalogPageProps) {
  ...
  let siblingGroups: Record<string, McpServer[]> = {};
  try {
    ...
    submittedMcps = await getMcpServers().catch(() => []);
    // 预聚合每个已提交 MCP 的同 group 完整卡片数据（含激活版 + 其它已上架版本）。
    // 列表仍每 group 只下发激活版一张卡，其余版本在此预取，供卡片标题版本下拉「就地切换」。
    ...
    const groupResults = await Promise.all(
      groupKeys.map(async (g) => {
        ...
        const activeCards = submittedMcps.filter(
          (m) => (m.group_key || String(m.payload_ref || "").split(":")[0] || "") === g,
        );
        const have = new Set(activeCards.map((m) => m.id));
        const siblingIds = onShelfIds.filter((id) => !have.has(id));
        const fetched = await Promise.all(
          siblingIds.map((id) => getMcpById(id).catch(() => null)),
        );
        return {
          g,
          cards: [...activeCards, ...fetched.filter((x): x is McpServer => !!x)],
        };
      }),
    );
